# Afya Triage Microservice

An Emergency Severity Index (ESI)-aligned clinical risk assessment microservice built for low-resource deployment environments.

---

## 📖 The Story Behind Afya Triage

***Afya*** is the Swahili word for ***Health***.

In rural clinics and under-resourced emergency departments across East Africa, healthcare workers face severe staffing shortages and overwhelming patient surges. Delayed recognition of critically ill individuals—particularly patients with silent hypoxia or unstable vital signs—frequently leads to preventable morbidity and mortality.

Afya Triage was built as a zero-dependency, deterministic clinical triage engine capable of running on low-cost edge devices and Android hardware via Termux. It prioritizes patients into 4 clinical urgency levels and enforces strict safety overrides for severe hypoxia (SpO2 < 88%), ensuring vital risk decisions occur in milliseconds even during power or internet grid failures.

---

## 📊 Analytics & Distribution Visualizations

![Afya Triage Population Distribution](docs/triage_distribution.svg)

### Data Source

The visualization above reflects evaluation metrics across a synthetic population sample (N=1000) generated by [`src/dataset.py`](https://github.com/jamesruga/afya-triage/blob/main/src/dataset.py). The dataset models typical clinical presentations in emergency triage settings across six key vital signs: Systolic BP, Diastolic BP, Heart Rate, Respiratory Rate, SpO2, and Temperature.

### How to Read

* Level 1 (Critical - Red): Immediate life-saving intervention needed. Represents acute respiratory failure or severe clinical instability (SpO2 < 88%).
* Level 2 (Emergent - Orange): High-risk presentation with severely abnormal vitals requiring urgent medical evaluation.
* Level 3 (Urgent - Yellow): Moderate vital sign abnormalities requiring standard clinical management.
* Level 4 (Non-urgent - Green): Low-risk or stable vital signs suited for routine care queues.

---

## 🚀 Key Features

* Deterministic Risk Engine: Weighted clinical scoring based on vital signs with hard safety overrides for acute hypoxia (SpO2 < 88%).
* Dual API Specs: Supports both single-patient real-time assessment and array-based batch processing.
* Production-Ready Stack: Powered by Flask WSGI behind Gunicorn with multi-worker concurrency.

---

## 📦 Project Structure

| Path | Description |
| :--- | :--- |
| .github/workflows/ci.yml | GitHub Actions CI testing pipeline |
| .github/workflows/telemetry-sync.yml | Daily system telemetry heartbeat workflow |
| data/ | Synthetic patient data store |
| docs/ | SVG visualizations & performance distribution charts |
| models/ | Model evaluation metadata & classification thresholds |
| src/api.py | Flask REST API endpoints |
| [`src/dataset.py`](https://github.com/jamesruga/afya-triage/blob/main/src/dataset.py) | Synthetic vital signs population generator |
| src/triage.py | Core ESI clinical classification engine |
| src/visualize.py | SVG visualization engine |
| tests/test_triage.py | Pytest suite for clinical logic & API validation |
| Dockerfile | Container configuration for edge/cloud deployment |
| requirements.txt | Minimal Python dependencies |
| start_server.sh | Production Gunicorn WSGI startup script |

---

## 🛠️ Local Setup & Testing

1. Install dependencies:

```bash
pip install -r requirements.txt
```

2. Run test suite:

```bash
PYTHONPATH=. pytest -v
```

3. Generate visualizations:

```bash
PYTHONPATH=. python3 src/visualize.py
```

4. Start production server:

```bash
chmod +x start_server.sh
./start_server.sh
```

---

## 📡 API Endpoint Reference

### Single Patient Assessment (POST /triage)

```bash
curl -X POST [http://127.0.0.1:8000/triage](http://127.0.0.1:8000/triage) \\
  -H "Content-Type: application/json" \\
  -d '{
    "systolic_bp": 120,
    "diastolic_bp": 80,
    "heart_rate": 75,
    "respiratory_rate": 16,
    "oxygen_saturation": 85,
    "temperature": 36.6
  }'
```

### Batch Patient Processing (POST /triage/batch)

```bash
curl -X POST [http://127.0.0.1:8000/triage/batch](http://127.0.0.1:8000/triage/batch) \\
  -H "Content-Type: application/json" \\
  -d '[
    {
      "systolic_bp": 120,
      "diastolic_bp": 80,
      "heart_rate": 75,
      "respiratory_rate": 16,
      "oxygen_saturation": 98,
      "temperature": 36.6
    },
    {
      "systolic_bp": 118,
      "diastolic_bp": 76,
      "heart_rate": 70,
      "respiratory_rate": 14,
      "oxygen_saturation": 84,
      "temperature": 36.5
    }
  ]'
```

---

## 🐳 Container Deployment

1. Build the image:

```bash
docker build -t afya-triage:latest .
```

2. Run the container:

```bash
docker run -d -p 8000:8000 --name afya-triage-service afya-triage:latest
```
